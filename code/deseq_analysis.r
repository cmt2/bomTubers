...
annotated <- read.csv(file = "~/Documents/trans_diff/trans_annotate/trinotate_annotation_report.csv",
...
results(dds, contrast = c("condition", "tub","roo"),
  ...
  mutate(color = ifelse(.$padj < 0.01 &
                          abs(.$log2FoldChange) > 2, "darkred","black")) %>%
  mutate(abs_log2foldchange = abs(.$log2FoldChange)) -> res

plot(res$abs_log2foldchange, pch = 20, col = alpha("black",0.3),
     ylab = "Absolute Value of Log2 Fold Change", xlab = NA,
     xaxt = 'n',
